...
public class RegUtil {
    ...
    private static final String PHONE_REG = "^[1](([3][0-9])|([4][5-9])|([5][0-3,5-9])|([6][5,6])|([7][0-8])|([8][0-9])|([9][1,8,9]))[0-9]{8}$";//手机号正则表达式
    ...
    private static String contertToNewCardNumber(String oldCardNumber){
        ...
        buf.append(RegUtil.calculateVerifyCode(buf));
        return buf.toString();
    }

    public static boolean isStudenNum(String studentnum){
        if(studentnum==null){
            return false;
        }
        return studentnum.matches("[0-9]{10}");
    }
